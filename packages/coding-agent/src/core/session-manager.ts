...
function assertExpectedPathType(filePath: string, stats: Stats, expected: "file" | "directory"): void {
...
function writePrivateSessionFile(filePath: string, content: string, mode: "append" | "exclusive"): void {
	...
	try {
		...
		if (!sameFileIdentity(current, opened))
			throw new Error(`Session file changed while creating: ${resolvedFilePath}`);
...
function readSessionHeader(filePath: string): SessionHeader | null {
	const fd = openSessionFileForRead(filePath);
	try {
		const decoder = new StringDecoder("utf8");
